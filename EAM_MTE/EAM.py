...
def eam_encriptar(mensagem, chave):
    ...
    texto_cifrado = cifra.encrypt(padding(mensagem))  # Texto simples encripitado sem o mac
    return cifra.IV + texto_cifrado + tag  # Retrona a mensagem encriptada


def eam_decriptar(cifrado, chave):
    ivd = cifrado[:16]  # ivd recebe o iv da mensagem encriptada
    tagd = cifrado[-32:]  # tagd recebe a tag da mensagem encriptada
    cifra = AES.new(chave, AES.MODE_CBC, ivd)  # Seleciona o modo de operacao
    msge = cifra.decrypt(cifrado)  # Primeiro decripta a msg
    texto_cifradod = msge[16: -32]  # Seleciona a msg
    texto = dpadding(texto_cifradod)  # Limpa o padding para descobrir a mensagem
    hmac = HMAC.new(chave, texto, SHA256)  # A partir da mensagem deriptada
    try:  # Verifica o mac
        hmac.verify(tagd)
    except ValueError:
...
msg = str.encode(a)  # Trasforma em bytes
key = get_random_bytes(16)  # Gera a chave
c = eam_encriptar(msg, key)  # Encripta a mensagem
iv = c[:16]  # Gera o iv
tg = c[-32:]  # Gera a tag
...
msgdecri = eam_decriptar(c, key)  # Desemcripta a mensagem
